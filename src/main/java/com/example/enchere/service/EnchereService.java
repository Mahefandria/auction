...
public class EnchereService {
    ...
    public List<EnchereDTO> findAllWithCurrentPrice2(String desc, Double prixMin, Double prixMax) {
        StringBuilder criteriaBuilder = new StringBuilder("WHERE 1=1");
        System.out.println();
        if (desc != null) {
            if (desc != "") {
                criteriaBuilder.append(" AND description like ").append("'%" + desc + "%'");
            }
        }
        if (prixMin != null) {
            if (prixMin > 0) {
                criteriaBuilder.append(" AND prix_courant >=").append(prixMin);
            }
        }
        if (prixMax != null) {
            if (prixMax > 0) {
                criteriaBuilder.append(" AND prix_courant <=").append(prixMax);
            }
        }
        return dtoRepository.findAuctionWithCurrentPrice(criteriaBuilder.toString());
    }
